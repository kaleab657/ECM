...
async function startServer() {
  ...
  // [SECURITY] Trust first proxy (Render reverse proxy) so req.ip returns real client IP
  ...
  // Tracks request counts per key (IP or user UID) with automatic expiry.
  const rateLimitStore = new Map<string, { count: number; resetTime: number }>();

  // Auto-cleanup expired entries every 5 minutes to prevent memory growth
  setInterval(() => {
    const now = Date.now();
  ...
  const createRateLimit = (windowMs: number, max: number, message?: string) => {
    return (req: any, res: any, next: any) => {
      // Use authenticated user UID if available, otherwise fall back to IP
      const userKey = req.user?.uid || '';
      const ipKey = req.ip || req.socket?.remoteAddress || 'unknown';
      const key = `${userKey || ipKey}:${max}`; // Include max in key so tiers don't collide

      const now = Date.now();
      const record = rateLimitStore.get(key);

      if (!record || now > record.resetTime) {
        // First request in this window — allow and start counting
  ...
  // Rate limit tiers
  const generalApiLimit = createRateLimit(60 * 1000, 100, 'Too many requests. Please slow down.');
  const writeApiLimit   = createRateLimit(60 * 1000, 20,  'Too many write requests. Please wait a moment.');
  const uploadLimit     = createRateLimit(60 * 1000, 10,  'Upload limit reached. Please wait before uploading again.');
  ...
  app.use((req, res, next) => {
    ...
    const scriptSrc = isProduction
      ? `script-src 'self' 'nonce-${nonce}' 'wasm-unsafe-eval' https://apis.google.com https://www.gstatic.com https://www.google-analytics.com https://firebaseinstallations.googleapis.com`
  ...
  app.post("/api/r2/upload-listing", uploadLimit, authenticate, express.raw({ type: "*/*", limit: "20mb" }), async (req, res) => {
  ...
  app.post("/api/r2/upload-payment", uploadLimit, authenticate, express.raw({ type: "*/*", limit: "20mb" }), async (req, res) => {
    let { fileName, fileType, customKey } = req.query as any;
  ...
  // [SECURITY] Apply general rate limit to ALL API routes (100 req/min per IP)
  api.use(generalApiLimit);

  // [SECURITY] Apply stricter write limit to all mutating methods
  api.use((req: any, res: any, next: any) => {
    if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method)) {
      return writeApiLimit(req, res, next);
    }
    next();
  });
