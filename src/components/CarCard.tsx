...
import { Calendar, Gauge, Fuel, Star, Settings } from 'lucide-react';
...
export const CarCard = React.memo<CarCardProps>(({ car, onClick, variant = 'search', priority = false }) => {
  const { t, appConfig } = useAppContext();
  ...
  return (
    ...
    >
      <div className="aspect-[4/3] w-full overflow-hidden relative">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          {car.packageType === 'featured' && (
            <div className="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-lg bg-brand text-white">
              {t('carCard.featured') || 'Featured'}
            </div>
          )}
          {car.packageType === 'premium' && appConfig?.premium_enabled !== false && (
            <div className="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-lg bg-amber-500 text-white">
              {t('carCard.premium') || 'Premium'}
            </div>
          )}
        </div>
      ...
      <div className="p-2.5">
        ...
        <div className="grid grid-cols-2 gap-y-1.5 gap-x-3 mb-2">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
            ...
            <span>{car.fuel ? (t(`common.${car.fuel.toLowerCase()}`) || car.fuel) : ''}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
            <Settings size={12} className="text-zinc-300" />
            <span className="truncate">{car.transmission ? (t(`common.${car.transmission.toLowerCase()}`) || car.transmission) : ''}</span>
          </div>
        </div>
