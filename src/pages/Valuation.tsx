...
export const Valuation: React.FC = () => {
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    const key = `${formData.make} ${formData.model}`;
    let basePrice = BASE_PRICES[key] || 2000000; // Default if not found
    ...
    // Mileage factor: -1% per 10k km
    const mileage = parseInt(formData.mileage);
    const mileageFactor = Math.max(0.7, 1 - (mileage / 10000 * 0.01));
    
    const estimatedValue = Math.round(basePrice * ageFactor * mileageFactor);
          ...
          {valuation ? (
            <div className="bg-brand/5 rounded-[32px] p-8 border border-brand/10 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <h2 className="text-5xl font-black text-brand tracking-tighter mb-4">
                {valuation.toLocaleString()} <span className="text-xl">ETB</span>
              </h2>
              <p className="text-zinc-500 dark:text-zinc-400 text-sm leading-relaxed mb-6">
                {t('valuation.info')}
              </p>
              <button className="w-full bg-zinc-900 dark:bg-white dark:text-zinc-900 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-all">
                {t('valuation.sellButton')} <ArrowRight size={18} />
              </button>
            </div>
          ) : (
